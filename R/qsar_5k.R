...
svm_pred <-predict(svm_cv$finalModel,newdata=as.data.frame(X1[,1:numberPC]))
...
######################################################################
plot.new()
venn.plot <- venn.diagram(list(comp_model$rf,comp_model$svm,comp_model$mlp), 
                          NULL, 
