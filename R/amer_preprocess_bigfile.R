...
load("C:/R-files/QSAR/neuralnet187.RData")   # load up trained neural network
...
valid <- validSDF(sdf5k); sdf5k <- sdf5k[valid] # remove invalid data if we have any.
length(sdf5k)
...
write.table(pred5k,"C:/R-files/QSAR/new5K.csv",sep=",",row.names=TRUE)
...
# use the filter function in dplyr to extract compounds with low model differences (between neural and amers)
# and high IC50 values - its a tradeoff. Write the list to file or an error message if we cant find any with 
# the settings in filter() function.
